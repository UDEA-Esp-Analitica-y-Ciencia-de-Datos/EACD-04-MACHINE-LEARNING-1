...
def test_clean_data(func):
    db = np.loadtxt('local/data/AirQuality.data',delimiter='\t')  # Assuming tab-delimiter
...
def experiementarSVR(func):
    ...
    gs = [1.0, 0.1]
    cs = [100]
    cols= ['kernel', 'gamma', 'param_reg', 'error de validación (promedio)',
       'error de validación (intervalo de confianza)', '% de vectores de soporte']

    cols_errs =['error de validación (promedio)']
    ...
    if ( (df_r['error de validación (intervalo de confianza)'] == df_r['error de validación (promedio)']).all()):
...
def experiementarSVC(func):
    ...
    cols= ['kernel', 'gamma', 'param_reg', 'error de entrenamiento',
       'error de validación', '% de vectores de soporte']
    
    cols_errs =['error de validación', 'error de entrenamiento']
